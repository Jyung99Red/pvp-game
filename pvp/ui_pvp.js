...
const uiPvp = (() => {
    ...
    function updateFrame(events = []) {
        ...
        const spMax = local.config.skillPointMax;
        $('pvp-self-sp').textContent = `${local.skillPoints} / ${spMax}${local.skillPoints < spMax && local.skillProgress > 0 ? ` · ${Math.floor(local.skillProgress * 100)}%` : ''}`;
