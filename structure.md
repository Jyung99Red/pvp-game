...
**判定规则是注册式的：** 五类交锋不再是 if/else 链，而是按优先级排序的
规则表(`registerExchangeRule({name, priority, when, resolve})`，clash 400 >
parry 300 > block 200 > interrupt 100 > hit 0 兜底)，`resolveExchange` 逐条
`when(ctx)` 取第一个命中的 `resolve(ctx)`。低优先级规则可以依赖高优先级
规则"没命中"这一事实(如 block 只会看到弹反窗口外的 guard_ready)。新增
机制 = 注册一条规则，判定结果原样走 PVP `result` 消息广播，协议无需改动
——但双方必须装载同一套规则，Host 才是唯一判定权威。
