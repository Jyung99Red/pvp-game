// ui_battle.js - Dedicated high-frequency battle view renderer

// Attach updateBattle to the global ui object
ui.updateBattle = function () {
    if (!state.battle.active) return;
    const b = state.battle;

    // 1. Battle end/settlement state toggle
    if (b.waitingChoice) {
    ...
    // 2. Player action button state updates
    ...
    if (nodeLeft) {
        ...
        // Only clear shield classes when neither winding up nor holding (avoids interrupting the animation every frame)
    ...
    // 4. Enemy info and HP bar update
    ...
    // 5. Enemy charge bar and animation control
    ...
    // 6. Hero HP bar, skill star rating
    ...
    // 7. Hero action-point bar update
...
// UI init at battle start (clear leftovers, refresh icons); called by battle.startFight
ui.initEnemy = function (eData, isFirst) {
    ...
    // Fade in on first battle entry; clear animation otherwise
